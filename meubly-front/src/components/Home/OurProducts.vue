  ...
  <section class="py-4" data-testid="products-section">
    <div class="w-full text-center">
      ...
      <div v-else class="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-6 justify-center">
        <ProductCard v-for="product in items" :key="product.furniture_id" :product="product"
          class="mx-auto w-full mt-2" />
      </div>

      <!-- Pagination (serveur) -->
      <div v-if="!loading && !error && totalPages > 1" class="mt-8">
        <Paginator :rows="limit" :totalRecords="total" @page="onPageChange" :first="(currentPage - 1) * limit" />
      </div>
    </div>
  </section>
</template>
